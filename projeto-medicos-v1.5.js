...
while (rodar) {
    ...
    switch (opcao) {
        ...
        case 2:
            ...
            cadastrarMedico(crmMedico, nomeMedico, especialidadeMedico);
            break;

        case 3:
            let pesquisaID = readline.questionInt("Digite o ID do médico que deseja buscar: ");
            pesquisarMedicos(pesquisaID);
...
function cadastrarMedico(crmMedico, nomeMedico, especialidadeMedico) {
    const medico = {
        id: ++gerador_id,
        crm: crmMedico,
        nome: nomeMedico,
...
function pesquisarMedicos(pesquisaID) {
    for (const m of medicos) {
        if (m.id == pesquisaID) {
